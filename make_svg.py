...
render_radar_svg() is the reusable piece: it draws the same radar geometry
for any 9-axis (label, value, is_new) list, so other tools (e.g. the
skill-level-interview skill's results page) render with an identical chart
instead of a hand-rolled second implementation. __main__ below is just this
file's own use of it — regenerating the default site radar.
"""
import math
import os
...
def render_radar_svg(skills, clusters, title, subtitle):
    """Render the 9-axis radar chart as a standalone SVG string.

    skills: list of (label, value 0-4, is_new bool), clockwise from top.
    clusters: list of (name, first_axis_idx, last_axis_idx, color).
    """
    n = len(skills)
...
-->""")
    ...
    out.append(f'<text x="680" y="56" text-anchor="middle" font-size="32" font-weight="700" fill="#0b0b0b">{esc(title)}</text>')
    out.append(f'<text x="680" y="86" text-anchor="middle" font-size="18" fill="#52514e">{esc(subtitle)}</text>')
    out.append('</g>')
    ...
    widths = [10.5 * len(name) + 42 for name, *_ in clusters]
    total = sum(widths)
    lx = CX - total / 2
    ly = 926
    ...
    out.append('<!-- profile values: ' + ", ".join(f"{name}={v}" for name, v, _ in skills) + ' -->')
...
if __name__ == "__main__":
    ...
    svg = render_radar_svg(SKILLS, CLUSTERS, "Data Scientist Skills Inventory", "2026 revision — 9 axes, scale 0–4")
